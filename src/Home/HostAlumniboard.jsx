      ...
      {isLoading ? (
        ...
        <div>
            ...
            <tbody>
              {active.slice(startIndex, endIndex).map((el) => (
                <tr key={el.id}>
                  ...
                  <td>
                    <Link to={`/alumni/${el.id}`} className="text-decoration-none">
                      {el.First_name}
                    </Link>
                  </td>
          ...
          <p className="mt-5 text-nowrap fw-bold">
            Total alumni {active.length > 1? "are" : "is"} {active.length}.
          </p>
          {active.length > 5 && (
...
export default HostAlumniboard;
